...
//    HashSet<String> ephIDs = E

    EphID ephIDClass = com.example.android.bluetoothadvertisements.EphID.getInstance();
    ...
    public View getView(int position, View view, ViewGroup parent) {
        ...
        String ephID = new String(scanResult.getScanRecord().getServiceData(scanResult.getScanRecord().getServiceUuids().get(0)));
        if (!ephIDClass.ephIDs.contains(ephID)) {
            Toast.makeText(mContext, "EphID received: " + ephID, Toast.LENGTH_LONG).show();
            ephIDClass.ephIDs.add(ephID);
        }

        String name = scanResult.getDevice().getName();
        if (name == null) {
//            name = mContext.getResources().getString(R.string.no_name);
            name = mContext.getResources().getString(R.string.blank_string);
        }
